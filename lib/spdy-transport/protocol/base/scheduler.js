...
function insertCompare(a, b) {
  return a.priority.from === b.priority.from ?
      a.priority.to - b.priority.to :
      a.priority.from - b.priority.from;
}
...
Scheduler.prototype._write = function _write(data, enc, cb) {
  ...
  if (index >= this.list.length ||
      insertCompare(this.list[index], item) !== 0)
...
function tickCompare(marker, item) {
  return item.priority.from > marker ? -1 :
         item.priority.to < marker ? 1 :
         0;
}
...
Scheduler.prototype.tickAsync = function tickAsync() {
  ...
  var list = this.list;
  while (list.length > 0) {
    var marker = this.random();
    var index = utils.binaryLookup(list, marker, tickCompare);
