...
Connection.prototype._handleFrame = function _handleFrame(frame) {
  ...
  if (!stream && frame.id !== undefined) {
    ...
    var nonMonotonic = frame.id < state.stream.lastId.received;
    ...
    if (!stream &&
        ...
        state.version === 4 &&
        frame.id !== 0 &&
        (frame.id % 2 === 0 || nonMonotonic)) {
      state.debug('id=0 server received invalid stream ID', frame.id);
      this._goaway({
        lastId: state.stream.lastId.both,
        code: 'PROTOCOL_ERROR',
        send: true
    ...
    if (!stream && nonMonotonic) {
      state.debug('id=0 stream=%d closed', frame.id);
      this._goaway({
        lastId: state.stream.lastId.both,
        code: 'STREAM_CLOSED',
        send: true
    ...
    // Fail if not found
    if (!stream && frame.type !== 'HEADERS' && frame.type !== 'PRIORITY') {
