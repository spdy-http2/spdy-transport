...
Connection.prototype._handleHeaders = function _handleHeaders(frame) {
  ...
  // Must be some trailing HEADERS after close
  if (frame.id <= state.stream.lastId) {
    state.debug('id=%d headers after GOAWAY', frame.id);
    return;
  }
...
Connection.prototype.pushPromise = function pushPromise(parent, uri, callback) {
  ...
  // TODO(indutny): always return stream
  if (!stream) {
    process.nextTick(function() {
      if (callback)
