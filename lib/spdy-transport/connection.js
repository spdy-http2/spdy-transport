...
Connection.prototype._goaway = function _goaway(lastId) {
  var state = this._spdyState;
...
Connection.prototype._onStreamDrain = function _onStreamDrain() {
  var state = this._spdyState;
...
Connection.prototype.end = function end(callback) {
  ...
  this._goaway(state.stream.lastId);

  state.framer.goawayFrame(state.goaway, state.constants.goaway.OK);
};
