...
Stream.prototype._resetTimeout = function _resetTimeout() {
  var state = this._spdyState;
  ...
  if (state.timeout.delay === 0)
    return;

  var self = this;
  state.timeout.timer = setTimeout(function() {
    state.timeout.timer = null;
    self.emit('timeout');
  }, state.timeout.delay);
};
