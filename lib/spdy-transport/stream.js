...
function Stream(connection, options) {
  ...
  // TODO(indutny): track priority
  state.priority = options.priority | 0;
...
Stream.prototype._read = function _read() {
  ...
  // See: https://github.com/nodejs/readable-stream/issues/148
  if (this._readableState.length >= this._readableState.lowWaterMark)
    return;
